...
if ($_SERVER['REQUEST_METHOD'] === "GET") {
    ...
    $paymentMethod = isset($_GET['payment_method']) ? $_GET['payment_method'] : 'Cash';  // Default payment method
    ...
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO Reservations (user_id, event_id, ticket_id, status, payment_status, payment_method) VALUES (?, ?, ?, 'pending', 'pending', ?)");
        $stmt->execute([$userId, $eventId, $ticketId, $paymentMethod]);
        ...
        // Success message
        $_SESSION["reservation_success"] = "<div class='alert alert-success text-center'>
        ...
        $pdo->rollBack();
        $_SESSION["reservation_error"] = "<div class='alert alert-danger text-center'>
